...
export default function ProductDetailPage() {
  ...
  const { data, error, isLoading } = useApiResource<DataProduct>(
    `/api/core/data-products/${params.id}/`,
  );
    ...
    <div>
      ...
      {error && (
        <p className="mt-4 text-sm text-red-600">
          Could not load this data product ({error.status}).
        </p>
      )}
      ...
      {data && (
        <>
          ...
          <p className="mt-1 text-sm text-slate-600">
            {data.purpose || "No purpose recorded."}
          </p>
          ...
          <dl className="mt-8 grid grid-cols-2 gap-6 rounded-lg border border-slate-200 bg-white p-6 sm:grid-cols-3">
            ...
            <Field
              ...
              value={
                data.refresh_date
                  ? new Date(data.refresh_date).toLocaleString()
                  : ""
              }
            />
            <Field
            ...
            <Field
              ...
              value={
                data.permitted_audience.length > 0
                  ? data.permitted_audience.join(", ")
                  : ""
              }
            />
            <Field
              label="Governance review"
              value={data.governance_reviewed ? "Reviewed" : "Pending review"}
            />
          </dl>
          ...
          {data.quality_checks.length > 0 && (
            <div className="mt-6 rounded-lg border border-slate-200 bg-white p-6">
              ...
              <p className="mt-1 text-xs text-slate-500">
                Each check is labeled by method: deterministic (a hard fact),
                heuristic (a statistical judgement call), or a human approval
                that code never evaluates on its own.
              </p>
                  ...
                  >
                    ...
                    <p className="mt-1 text-xs text-slate-500">
                      {source.extraction_date &&
                        `Extracted ${source.extraction_date}`}
                      {source.extraction_date &&
                        source.reference_period &&
                        " · "}
                      {source.reference_period &&
                        `Reference period: ${source.reference_period}`}
                    </p>
                  </li>
                ))}
              </ul>
