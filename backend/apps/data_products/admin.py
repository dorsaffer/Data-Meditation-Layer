...
class DataProductAdmin(admin.ModelAdmin):
    ...
    permitted_audience) are editable here — that's the intended way to
    make the "restricted/admin-only" defaults a deliberate human
    decision. Pipeline-derived fields are read-only since
    sync_data_product() would just overwrite manual edits to them.
    """
    list_display = (
